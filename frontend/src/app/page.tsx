...
export default function Home() {
  ...
  const runComparison = async () => {
    ...
    const results = await Promise.all(
      targets.map(id =>
        fetch(`${API}/pipeline/${id}`, { method: "POST" }).then(r => r.json())
      )
    );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-400 text-sm mt-1">
            BFS Traversal + 5-Check Filter Pipeline — Zero LLM · Deterministic
          </p>
        </div>
        ...
        <div className="flex gap-2 mb-6">
          {(["pipeline", "compare"] as const).map(tab => (
            ...
            >
              {tab === "pipeline" ? "Pipeline" : "Compare Users"}
            </button>
          ))}
        </div>
        ...
        {activeTab === "pipeline" && (
          <>
              ...
              <div className="flex items-center gap-2 mb-6">
                {[
                  ...
                  <div key={i} className="flex items-center gap-2">
                    <div className={`rounded-lg p-4 text-center min-w-[100px] ${
                      box.green ? "bg-green-900/40 border border-green-700" : "bg-gray-800"
                    }`}>
                      <div className={`text-2xl font-bold ${box.green ? "text-green-400" : "text-white"}`}>
                        {box.value}
                      </div>
            ...
            {/* Candidate Set */}
            {result && <CandidateTable nodes={result.candidate_set} />}

            {!result && !loading && (
              <div className="text-center text-gray-500 py-20">
                Select a user and click Run Pipeline
              </div>
            )}
          </>
        )}
